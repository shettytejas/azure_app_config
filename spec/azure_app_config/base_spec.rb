    ...
    describe 'without params' do
      ...
      it 'returns all flags' do
        VCR.use_cassette('all/without_params') do
          ...
          expect(result.first.keys).to match_array ["etag", "key", "label", "content_type", "value", "tags", "locked", "last_modified"]
          expect(result.first['value'].keys).to match_array ["conditions", "description", "enabled", "id"]
        end
      end
    end
    ...
    describe 'flag with name exists' do
      ...
      it 'returns the flag' do
        VCR.use_cassette('fetch/with_existing_name_param_without_label') do
          ...
          expect(result.keys).to match_array ["etag", "key", "label", "content_type", "value", "tags", "locked", "last_modified"]
          expect(result['value'].keys).to match_array ["conditions", "description", "enabled", "id"]
        end
      end
    end
